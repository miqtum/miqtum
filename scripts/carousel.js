...
const CLOSED = true;
const OFFSET = -12;           // смещение камеры от трубки вдоль нормали
const LOOP_TIME_MS = 80000;  // полный оборот за 40 секунд
const TUBE_WORLD_RADIUS = RADIUS * SCALE;

// Управление временем (пауза/продолжить)
let isPaused = false;
const timeline = { start: performance.now(), elapsed: 0 };
...
function init() {
  ...
  const modelsOnCurve =[
    ...
    { url: './models/SCOOF.glb', t: 0.88, size: 22 }
  ];
...
// Основной цикл. Используем свой таймер, чтобы уметь ставить на паузу.
function animate() {
  if (!isPaused) {
    timeline.elapsed = performance.now() - timeline.start;
  }
  render(timeline.elapsed);
}

function render(elapsedMs) {
  const t = ((elapsedMs % LOOP_TIME_MS) / LOOP_TIME_MS);
...
function createPauseButton() {
  ...
  pauseBtn.addEventListener('mouseleave', () => (pauseBtn.style.opacity = '0.9'));

  pauseBtn.addEventListener('click', () => {
    if (!isPaused) {
      // Ставим на паузу: фиксируем elapsed
      isPaused = true;
      pauseBtn.textContent = 'Resume';
    } else {
      // Снимаем с паузы: подгоняем start так, чтобы elapsed продолжал увеличиваться с текущего значения
      isPaused = false;
      timeline.start = performance.now() - timeline.elapsed;
      pauseBtn.textContent = 'Stop';
    }
  });
